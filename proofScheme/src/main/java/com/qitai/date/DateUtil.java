...
public class DateUtil {
    ...
    public static int isWeekend(String bDate) throws ParseException {
    ...
    public static List getHolidays(int year) throws ParseException {
        List<String> holidays = new ArrayList<>();
        List<String> transfer_to_rest = new ArrayList<>();
        List<String> gregorianCalendarHolidays = new ArrayList<>();
        gregorianCalendarHolidays.add(year+"-01-01");
        gregorianCalendarHolidays.add(year+"-05-01");
        gregorianCalendarHolidays.add(year+"-10-01");
        gregorianCalendarHolidays.add(year+"-10-02");
        gregorianCalendarHolidays.add(year+"-10-03");

        List<String> lunarCalendarHolidays = new ArrayList<>();
        lunarCalendarHolidays.add(year+"-01-01");
        lunarCalendarHolidays.add(year+"-01-02");
        lunarCalendarHolidays.add(year+"-01-03");
        lunarCalendarHolidays.add(year+"-05-05");
        lunarCalendarHolidays.add(year+"-08-15");

        String new_year_day = year+"-01-01";
        int day_of_week = isWeekend(new_year_day);
        //元旦 逢周一、周三、周五不调休，逢周二将上周六假日调至周一，逢周四将本周日假期调至周五，逢周末下周一补休
        if (day_of_week==3){
            String preWeek_Saturday = lessDate(new_year_day,3);
            transfer_to_rest.add(preWeek_Saturday);

            String preWeek_Monday = lessDate(new_year_day,1);
            transfer_to_rest.add(preWeek_Monday);
        }

        return holidays;
    }

    public static String lessDate(String date,int day) throws ParseException {
        ...
        cal.set(Calendar.DAY_OF_MONTH, cal.get(Calendar.DAY_OF_MONTH) - day);// 让日期减去day
        Date endDate = cal.getTime();
        String formatDate = cn.hutool.core.date.DateUtil.formatDate(endDate);
        return formatDate;
    }
}